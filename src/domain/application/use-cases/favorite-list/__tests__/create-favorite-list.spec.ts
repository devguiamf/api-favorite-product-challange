...
import { CreateFavoriteListRequest, CreateFavoriteListUseCase } from "../create-favorite-list";
...
function makeCreateFavoriteListRequest(modifications?: Partial<CreateFavoriteListRequest>): CreateFavoriteListRequest {
  return {
...
describe(CreateFavoriteListUseCase.name, () => {
    ...
    it('should throw an error when the list favorite already exists', async () => {
        ...
        await expect(sut.execute(createFavoriteListRequest)).rejects.toThrow('Já existe uma lista de favoritos para este usuário');
    });
});